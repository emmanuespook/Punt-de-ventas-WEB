...
<option></option>
	...
	$result = $db->prepare("SELECT * FROM products WHERE warehouse='$warehouse' and qty>0");
		$result->bindParam(':userid', $res);
		$result->execute();
		for($i=0; $row = $result->fetch(); $i++){
	?>
		<option value="<?php echo $row['product_id'];?>"><?php echo $row['product_code']; ?> - <?php echo $row['gen_name']; ?> - <?php echo $row['product_name']; ?>|| Cant.disponible: <?php echo $row['qty']; ?> </option>
	<?php
				}
			?>
<option disabled>----------EN OTROS ALMACÉNES---------</option>
    <?php
	include('../connect.php');
	$result = $db->prepare("SELECT * FROM products WHERE warehouse!='$warehouse'");
		$result->bindParam(':userid', $res);
		$result->execute();
		for($i=0; $row = $result->fetch(); $i++){
	?>
		<option disabled value="<?php echo $row['product_id'];?>"><?php echo $row['product_code']; ?> - <?php echo $row['gen_name']; ?> - <?php echo $row['product_name']; ?> ||  Almacén : <?php echo $row['warehouse']; ?> </option>
	<?php
				}
			?>
