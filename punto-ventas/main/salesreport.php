...
<div class="container-fluid">
	...
	<div class="contentheader">
			...
			<li class="active">Reporte de Ventas / <?php echo $warehouse; ?></li>
			</ul>
...
<button  style="float:right;" class="btn btn-success"><a href="javascript:Clickheretoprint()"> Imprimir</button></a>

</div>
<form action="salesreport.php" method="get">
<center><strong style="color:white;">De : <input type="text" style="width: 223px; padding:14px;" name="d1" class="tcal" value="" /> A: <input type="text" style="width: 223px; padding:14px;" name="d2" class="tcal" value="" />
    <input type="hidden" style="width: 223px; padding:14px;" name="ware" class="tcal" value="<?php echo $warehouse; ?>" />
...
<div style="font-weight:bold; text-align:center;font-size:14px;margin-bottom: 15px; color:white;">
...
<table class="table table-bordered" id="resultTable" data-responsive="table" style="text-align: left;">
	...
	<tbody>
			...
			<?php
				...
				$d1=$_GET['d1'];
				$d2=$_GET['d2'];
				$result = $db->prepare("SELECT s.*,so.product_code,so.gen_name FROM sales as s, sales_order as so WHERE s.date BETWEEN :a AND :b AND s.warehouse='$warehouse' AND s.invoice_number=so.invoice ORDER by transaction_id  DESC ");
	...
	<thead>
		<tr>
			...
			<?php
				...
				$d1=$_GET['d1'];
				$d2=$_GET['d2'];
				$results = $db->prepare("SELECT sum(amount) FROM sales WHERE date BETWEEN :a AND :b AND warehouse='$warehouse'");
				...
				<th colspan="1" style="border-top:1px solid #999999">
			<?php 
				$resultia = $db->prepare("SELECT sum(profit) FROM sales WHERE date BETWEEN :c AND :d AND warehouse='$warehouse'");
