...
<a  href="index.php"><button class="btn btn-default btn-large" style="float: left;"><i class="icon icon-circle-arrow-left icon-large"></i> Volver</button></a>
			...
			<?php 
			include('../connect.php');
				$result = $db->prepare("SELECT * FROM products where qty < 2 ORDER BY product_id DESC");
				$result->execute();
			...
			<div style="text-align:center;">
			<font style="color:rgb(255, 95, 66);; font:bold 22px 'Aleo';">[<?php echo $rowcount123;?>]</font> Productos estan por debajo de 2 Und. 			</div>
</div>
...
<table class="hoverTable" id="resultTable" data-responsive="table" style="text-align: left;">
	...
	<tbody>
			...
			function formatMoney($number, $fractional=false) {
				...
				include('../connect.php');
				$result = $db->prepare("SELECT *, price * qty as total FROM products ORDER BY product_code ASC");
				$result->execute();
			...
			<?php
			$total=$row['total'];
			$costoalmacen=$costoalmacen+$total;
...
<center><h2>Costo de almacén <?php echo "$".formatMoney($costoalmacen, true); ?></h2></center>
<br>
<div class="clearfix"></div>

<br>
<!------------- SEGUNDA TABLA DE EXISTENCIAS EN OTRO ALMACÉN 
<center><h3>PRODUCTOS EXISTENTES EN OTROS ALMACÉNES</h3></center>
<table class="hoverTable" id="resultTable" data-responsive="table" style="text-align: left;">
	<thead>
		<tr>
		    <th width="5%"> ID </th>
			...
			<th width="5%"> Cantidad Inicial</th>
			<th width="5%"> Cantidad Restante</th>
	...
	<tbody>
			...
			<?php
				...
				include('../connect.php');
				$result = $db->prepare("SELECT *, price * qty as total FROM products WHERE warehouse!='$warehouse' ORDER BY product_id DESC");
				$result->execute();
				for($i=0; $row = $result->fetch(); $i++){
				$total=$row['total'];
				$availableqty=$row['qty'];
