...
<span>Modelo : </span><input type="text" style="width:265px; height:30px;"  name="gen" value="<?php echo $row['gen_name']; ?>" /><br>
...
<span>Almacén : </span>
<select name="warehouse" style="width:265px; height:30px; margin-left:-5px;" >
	<option><?php echo $row['warehouse']; ?></option>
	<?php
	$resul = $db->prepare("SELECT * FROM warehouse");
		$resul->bindParam(':userid', $res);
		$resul->execute();
		for($k=0; $rowsk = $resul->fetch(); $k++){
	?>
		<option><?php echo $rowsk['warehouse_nickname']; ?></option>
	<?php
	}
	?>
</select><br>
